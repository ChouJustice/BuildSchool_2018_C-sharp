...
namespace Life_Number
{
    public partial class Form1 : Form
    {
        ...
        {
            string value = dateTimePicker1.Value.Year.ToString() + dateTimePicker1.Value.Month.ToString() + dateTimePicker1.Value.Day.ToString();

            int life_number = 0;
            //value = life_number.ToString();
            do
            {
                for (int i = 0; i < value.Length; i++)
                {
                    life_number += Convert.ToInt32(value[i]) - '0';
                }
                //label2.Text += life_number.ToString() + Environment.NewLine;
                value = life_number.ToString();
                life_number = 0;
            }
            while (value.Length > 1);

            life_number = Convert.ToInt32(value);

            star = Star_identify();

            label1.Text = "你的星座是 : " + star + Environment.NewLine + "你的生命靈數是 " + life_number.ToString() + " : ";

            Dictionary<int, string> main_list = new Dictionary<int, string>();

            main_list = Create_Dictionary();

            foreach(var item in main_list)
            {
                if(item.Key == life_number)
                {
        ...
        private Dictionary<int, string> Create_Dictionary()
        {
        ...
        private string Star_identify()
        {
            int year = dateTimePicker1.Value.Year;

            DateTime first_Aries = new DateTime(year, 3, 21);
            DateTime end_Aries = new DateTime(year, 4, 19);
            DateTime first_Taurus = new DateTime(year, 4, 20);
            DateTime end_Taurus = new DateTime(year, 5, 20);
            DateTime first_Gemini = new DateTime(year, 5, 21);
            DateTime end_Gemini = new DateTime(year, 6, 21);
            DateTime first_Cancer = new DateTime(year, 6, 22);
            DateTime end_Cancer = new DateTime(year, 7, 22);
            DateTime first_Leo = new DateTime(year, 7, 23);
            DateTime end_Leo = new DateTime(year, 8, 22);
            DateTime first_Virgo = new DateTime(year, 8, 23);
            DateTime end_Virgo = new DateTime(year, 9, 22);
            DateTime first_Libra = new DateTime(year, 9, 23);
            DateTime end_Libra = new DateTime(year, 10, 23);
            DateTime first_Scorpio = new DateTime(year, 10, 24);
            DateTime end_Scorpio = new DateTime(year, 11, 21);
            DateTime first_Sagittarius = new DateTime(year, 11, 22);
            ...
            DateTime end_Aquarius = new DateTime(year, 2, 19);
            DateTime first_Pieces = new DateTime(year, 2, 20);
            DateTime end_Pieces = new DateTime(year, 3, 20);

            var star_date = dateTimePicker1.Value.Date;

            if(first_Aries <= star_date && star_date <= end_Aries)
            {
                return "牡羊座";
            }
            else if(first_Taurus <= star_date && star_date <= end_Taurus)
            {
                return "金牛座";
            }
            else if (first_Gemini <= star_date && star_date <= end_Gemini)
            {
                return "雙子座";
            }
            else if (first_Cancer <= star_date && star_date <= end_Cancer)
            {
                return "巨蟹座";
            }
            else if (first_Leo <= star_date && star_date <= end_Leo)
            {
                return "獅子座";
            }
            else if (first_Virgo <= star_date && star_date <= end_Virgo)
            {
                return "處女座";
            }
            else if (first_Libra <= star_date && star_date <= end_Libra)
            {
                return "天秤座";
            }
            else if (first_Scorpio <= star_date && star_date <= end_Scorpio)
            {
                return "天蠍座";
            }
            else if (first_Sagittarius <= star_date && star_date <= end_Sagittarius)
            {
                return "射手座";
            }
            else if (first_Capricorn <= star_date && star_date <= end_Capricorn)
            {
                return "魔羯座";
            }
            else if (first_Aquarius <= star_date && star_date <= end_Aquarius)
            {
                return "水瓶座";
            }
            else if (first_Pieces <= star_date && star_date <= end_Pieces)
            {
                return "雙魚座";
            }
            else
